    ...
    if request.method == 'POST':
        try:
            datos = {
                ...
                'costo_base': parse_float(request.form.get('costo_base'), 'costo base', required=True, minimum=0),
                'tiempo_estimado_horas': parse_int(request.form.get('tiempo_estimado_horas'), 'tiempo estimado', required=True, minimum=1),
            }
...
def eliminar_servicio(id):
    ...
    flash('Servicio eliminado exitosamente' if resultado['exito'] else resultado['mensaje'], 'success' if resultado['exito'] else 'error')
    return redirect(url_for('servicio.listar_servicios'))
